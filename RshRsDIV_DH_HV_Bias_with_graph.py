...
def main():
    ...
    Read48Current.WriteToFile("Datetime: "+str(NOW)+"\n"\
    ...
    Read48Current.WriteToFile("Timepoint,Epoch Time (seconds), Time (hrs),"+  \
    "Sample Number, Rsh Mean, Rsh StDev, Rs Min, Sample Number, Rsh Mean, Rsh StDev, Rs Min,"+\
    "Sample Number, Rsh Mean, Rsh StDev, Rs Min, , , , , , , , , , , , , , , , , , , , , , , ,"+\
    ...
    for i in range(number_points):
        try:
            ...
            if i%points_between_LV == 0:
                ...
                for j in range NUMBER_SAMPLES: 
	                ...
	                LV_output[i,5] = np.std(DIV_Rsh_output[1:-1, 2])
               ...
               # 4 wire on second port
                ...
                LV_output[i,9] = np.std(DIV_Rsh_output[1:-1, 2])
                #raw_data_Rsh[i,0] = DIV_Rsh_output
                DIV_Rs_output, RS_MIN, terminal = DIVRs(
                RS_VOLTAGE_PROTECTION, RS_START_CURRENT,RS_STOP_CURRENT,RS_CURRENT_STEP,RS_NUMBER_STEPS,RS_SOURCE_DELAY
                )
               ...
               # 4 wire on third port
                ...
                time.sleep(0.1)
                LV_output[i,11] = 3
                LV_output[i,12] = RSH_MEAN
                LV_output[i,13] = np.std(DIV_Rsh_output[1:-1, 2])
                #raw_data_Rsh[i,0] = DIV_Rsh_output
                DIV_Rs_output, RS_MIN, terminal = DIVRs(
                RS_VOLTAGE_PROTECTION, RS_START_CURRENT,RS_STOP_CURRENT,RS_CURRENT_STEP,RS_NUMBER_STEPS,RS_SOURCE_DELAY
                )
                time.sleep(0.1)
                #raw_data_Rs[i,0] = DIV_Rs_output
                LV_output[i,14] = RS_MIN            
                serArduino.write('4O34O')
                time.sleep(0.1)
                Read48Current.WriteToFile(str(LV_output[i,0]) + "," + str(LV_output[i,1]) + "," + str(LV_output[i,2]) + "," + str(LV_output[i,3]) + "," + \
        ...
        'weight' : 'bold',
        'size'   : 14}
        title_font = {'fontname':'Arial', 'size':'12', 'color':'black', 'weight':'normal','verticalalignment':'bottom'} # Bottom vertical alignment for more space
        ...
        title(r'Rsh',**font)
        plt.legend(['Sample 1', 'Sample 2', 'Sample 3'], loc='upper left')
